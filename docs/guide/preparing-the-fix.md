...
- Backport to every supported line, and hold the patch until you are ready to publish (see [§7](/guide/coordinating-publication)).
...
If so, find the exact commit or version that fixes it and confirm it really does, ideally with a test that fails on the vulnerable version and passes on the fixed one. Then move on to [preparing the advisory](/guide/preparing-the-advisory).
...
Access is coarse: anyone you add as a collaborator on the advisory can reach the fork, and there is no finer control than that (see [LIMITATIONS.md](/limitations)). Add only the people you need. The reporter can also open their own private fork and push branches or pull requests into it; that is by design.
...
- **Do not skip your hooks.** Reaching for `--no-verify` to get past a pre-commit check is how unsigned or unchecked code slips into a security release (see the [cheatsheet gotchas](/cheatsheet#avoiding-the-gotchas)).
...
This happens when you ship, in [§7](/guide/coordinating-publication), but it shapes how you commit now, so know it in advance.
...
![A merged commit titled "Merge commit from fork", whose body lists the original fork commits as bullet points (regression tests and fixes, each referencing the GHSA advisory) followed by a co-author line.](/img/merge_commit_from_fork.png)
...
- **The history loses context.** Each branch gets one generic commit, which is why reconstructing the story afterward takes deliberate effort (see [§8](/guide/wrapping-up)).
