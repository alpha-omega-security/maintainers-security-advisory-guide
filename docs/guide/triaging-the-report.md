...
- Decide validity and scope here. How bad it is comes later, in [§6](/guide/preparing-the-advisory).
...
- **Accept** it and move on to [preparing the fix](/guide/preparing-the-fix).
- **Close** it as out of scope or not a vulnerability.
- **Redirect** it to the dependency or upstream project that owns it.
- **Rescope** it: real, but a hardening or a lower-impact issue rather than the vulnerability as reported. (How bad it is is a [scoring](/guide/preparing-the-advisory) question, not a triage one.)

:::tip
The moment you accept a report, you can reserve its CVE (see [§6](/guide/preparing-the-advisory)). Getting a number assigned can take time, so reserving early means it is ready when you publish, instead of becoming the thing everyone waits on at the end.
:::
...
![The GitHub security advisory triage view, showing the options a maintainer has on a privately reported vulnerability: "Start a temporary private fork", "Accept and open as a draft", a comment box, and a "Close security advisory" button.](/img/triage_options.png)

- **Start a temporary private fork** — collaborate on a fix privately (see [§5](/guide/preparing-the-fix)).
...
There is also no way to give reporters a submission template the way you can for issues or pull requests, so reports arrive in wildly varying shape and quality (see [LIMITATIONS.md](/limitations)).
...
Ideally you wrote it down before the first report arrived. If you have not, this report is the prompt to do it; the how-to lives in [§9](/guide/building-for-the-long-term).
...
:::note
Referencing an advisory from a pull request or issue does not create the automatic link-back you get between issues and PRs. If you discuss or track an advisory elsewhere, you have to keep those connections yourself. See [LIMITATIONS.md](/limitations).
:::
...
- **Stay calm if they disagree.** You have the final say on your own project, but a reporter who feels brushed off is the one who escalates, with a public issue, a blog post, or a CVE requested without you (see [§3](/guide/acknowledging-the-report)). A respectful rejection is also your reputation talking.
...
![A maintainer's closing comment on a security advisory: it thanks the reporter, explains the issue requires the attacker to control both arguments to the function so input validation is the calling application's responsibility, notes that this falls under the "Unvalidated Application Input" exclusion in the threat model, and closes the report as out of scope.](/img/close_out_of_scope.png)

The second declines a report that is not a real defect, explaining the reasoning plainly:

![A maintainer's closing comment on a security advisory: it explains that a 1,200-level deep nested object is a purely adversarial input, that every recursive function exhausts the call stack on sufficiently deep input, that this is a fundamental constraint of call stacks rather than a library defect, and thanks the reporter for disclosing responsibly.](/img/close_not_a_vulnerability.png)
...
- **Already known or already fixed.** You may not need to do anything but confirm the fixed version (see [§5](/guide/preparing-the-fix)).
